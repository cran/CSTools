...
CST_BEI_Weighting <- function(var_exp, aweights, terciles = NULL, 
  ...
  if (is.null(names(dim(var_exp$data))) || is.null(names(dim(aweights)))) {
    stop("Element 'data' from parameter 'var_exp' and parameter 'aweights'",
         " should have dimmension names.")
  }
  if(!(time_dim_name %in% names(dim(var_exp$data)))) {
    stop("Element 'data' from parameter 'var_exp' must have ",
         "temporal dimension.")
  }
  if(!(time_dim_name %in% names(dim(aweights)))) {
    stop("Parameter 'aweights' must have temporal dimension.")
  }
  if(!('member' %in% names(dim(var_exp$data)))) {
    stop("Element 'data' from parameter 'var_exp' must have ",
         "dimension 'member'.")
  }
  ...
  if (dim(var_exp$data)[time_dim_name] != dim(aweights)[time_dim_name]) {
    stop("Length of temporal dimensions ",
         "of element 'data' from parameter 'var_exp' and parameter ",
         "'aweights' must be equals.")
  }
  if (dim(var_exp$data)['member'] != dim(aweights)['member']) {
    stop("Length of dimension 'member' of element 'data' from ",
         "parameter 'var_exp' and parameter 'aweights' must be equals.")
  }
