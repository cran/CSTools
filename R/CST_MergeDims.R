...
#'
#'@description This function merges two dimensions of the array \code{data} in a 's2dv_cube' object into one. The user can select the dimensions to merge and provide the final name of the dimension. The user can select to remove NA values or keep them.
#'
#'@param data a 's2dv_cube' object
#'@param merge_dims a character vector indicating the names of the dimensions to merge
#'@param rename_dim a character string indicating the name of the output dimension. If left at NULL, the first dimension name provided in parameter \code{merge_dims} will be used.
...
#'dim(new_data$data)
#'@export
CST_MergeDims <- function(data, merge_dims = c('ftime', 'monthly'), rename_dim = NULL,
                         na.rm = FALSE) {
...
#'@author Nuria Perez-Zanon, \email{nuria.perez@bsc.es}
#'
#'@description This function merges two dimensions of an array into one. The user can select the dimensions to merge and provide the final name of the dimension. The user can select to remove NA values or keep them.
#'
...
#'@export
MergeDims <- function(data, merge_dims = c('time', 'monthly'), rename_dim = NULL,
                      na.rm = FALSE) {
    # check data
    if (is.null(data)) {
