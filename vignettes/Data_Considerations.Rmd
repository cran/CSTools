...
's2dv_cube' is a class of an object storing the data and metadata in several elements: 
  ...
  + $lat($lon) element is a vector indicating the latitudinal(longitudinal) values of the region in $data, 
  + $Variable describes the variable name and its units
...
```
library(CSTools)
class(lonlat_data$exp) # check the class of the object lonlat_data$exp
names(lonlat_data$exp) # shows the names of the elements in the object lonlat_data$exp
str(lonlat_data$exp) # shows the full structure of the object lonlat_data$exp
```
...
# 3.09 MB
library(s2dv)
PlotEquiMap(exp$data[1,1,1,1,,], lon = exp$lon, lat= exp$lat,
